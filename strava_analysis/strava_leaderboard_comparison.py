...
existing_data = []
try:
    with open('strava_analysis/leaderboards_urls.txt', 'r') as urlfile:
        for line in urlfile:
            existing_urls.append(line)
        existing_urls = [x.strip() for x in existing_urls]
    with open('leaderboards.json', 'r') as datafile:
...
with open('strava_analysis/leaderboards.json', 'w') as savefile:
    json.dump(leaderboards, savefile, indent=2, sort_keys=True)
with open('strava_analysis/leaderboards_urls.txt', 'w') as savefile:
